...
function compareSegs(seg1, seg2) {
	return seg1.eventStartMS - seg2.eventStartMS || // earlier events go first
		...
		seg2.event.allDay - seg1.event.allDay || // tie? put all-day events first (booleans cast to 0/1)
		 seg1.event.sortId - seg2.event.sortId; // 根据sortId排序,下面是原来的通过tilte首字母排序
...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			var time2 =slotDate.format(this.axisFormat);
			var a =  new Array();
			a = time2.split(":");
			var time1;
			if(a[1]==30){
				time1=  Number( a[0])+1 + ":00";
			}        else {
				time1 = a[0] + ":30";
			}
			var b =  new Array();
			b = this.exceptTime.split(":");
			var c =  new Array();
			c = this.exceptTimeTo.split(":");
			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
				((!slotNormal || !minutes) ?
					'<span>' + // for matchCellWidths
					htmlEscape(slotDate.format(this.axisFormat)) +  "-"+ time1    +
					'</span>' :
					'<span>' + // for matchCellWidths
					htmlEscape(slotDate.format(this.axisFormat)) +  "-"+ time1    +
					'</span>'
			...
			if(Number(b[0]+b[1])<=Number(a[0]+a[1]) && Number(a[0]+a[1])<Number(c[0]+c[1])){
				html +=
					'<tr  style="background-color: #F4F4F4"' + (!minutes ? '' : 'class="fc-minor"') + '>' +
					(!isRTL ? axisHtml : '') +
	...
	processOptions: function() {
		...
		this.exceptTime = view.opt('exceptTime');
		this.exceptTimeTo = view.opt('exceptTimeTo');
