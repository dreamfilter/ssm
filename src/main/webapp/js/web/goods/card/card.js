    ...
    var handleRecords = function () {
        ...
        dataTable.init({
            ...
            onQuery: function (data) {
                ...
                data.languageId = $.trim($("#languageIdQuery").getGAutoHiddenValue());
            },
            dataTable: {
                order: [],
                "ajax": {
                    "url": "w/goods/card/getCardListPage",
                    type: "POST"
        ...
        function initAddBtn() {
            $("#addBtn").on("click", function () {
                if ($("#dialogForm").validate().form()) {
                    ...
                    if(validatePeriod(nianka,sumPeriod)!=""){
                        toast.error(validatePeriod(nianka,sumPeriod));
                        return;
                    }
                    $.ajax({
...
function validatePeriod(nianka,sumPeriod) {
    var error="";
    if(nianka == 3){
        if(parseFloat(sumPeriod)%2 != 0 || parseFloat(sumPeriod)==0){
            error= "月卡课时数应为2的倍数！";
        }
    }
    if(nianka == 4){
    ...
    if(nianka == 5){
        if(parseFloat(sumPeriod)%6 != 0 || parseFloat(sumPeriod)==0){
            error= "年卡的课时数应为6的倍数！";
        }
    }
    return error;
